...
task ChainSwap {
  ...
  Int disk_size = ceil(size(selfref_bundle, "GB")) * 4
  ...
  command <<<
    ...
    for i in "~{d}{!sampleNames[@]}"; do
      ...
      this_sample=out/"~{d}{this_samp_name}"
      this_chain="~{d}{chains[i]}"

      chainSwap "~{d}{this_chain}" "~{d}{this_sample}~{target_suffix}_to_~{input_source_name}.chain"

      python ~{JsonTools} \
      --path out/jsonout.json \
      --set samples="~{d}{sampleNames[i]}" \
        chain="~{d}{this_sample}~{target_suffix}_to_~{input_source_name}.chain"
